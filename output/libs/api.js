var JSONbig, Q, doRequest, doRequests, fs, getURIComponent, request, syslog, url, _, _CONST_HOST_IP_, _CONST_HOST_NAME_, _CONST_NODE_ERROR_CODE_, _CONST_REMOTE_STATUS_CODE_;
...
getURIComponent = function(req) {
  var headers, key, options, urlComponent, urlStr, value, _ref;
  urlStr = req.url;
  urlComponent = {
    protocol: 'http:',
  ...
  _ref = urlComponent['query'];
  for (key in _ref) {
    value = _ref[key];
    if (urlComponent['query'][key] === void 0) {
      delete urlComponent['query'][key];
    }
  }
  headers = req['headers'] || {};
