module.exports = function anonymous($_data_) {
var $_output_ = '';var __$utils = this,$helper = this,__$include = __$utils.include,__LOCAL__ = this.__LOCAL__;
with($_data_ || {}) {
var $_output_ = '';$_output_ += '<div class="item role animated zoomIn">\n\t<h2>角色名：<span>';
$_output_ += item.name;
$_output_ += '</span>\n\t<p>';
$_output_ += item.description;
$_output_ += '</p>\n\t<div class="small red icon ui buttons menu">\n\t\t<div class="ui button settings" title="高级"><i class="settings icon"></i></div>\n\t\t<div class="ui button update" title="编辑"><i class="edit icon"></i></div>\n\t\t<div class="ui button delete" title="删除"><i class="minus  icon"></i></div>\n\t</div>\n\t</h2>\n\t<ul data-id="';
$_output_ += item.id;
$_output_ += '">\n\t\t';
 if (authorities.length == 0) { 
$_output_ += '\n\t\t<li class="empty">拖拽权限到这里！</li>\n\t\t';
 } else { 
$_output_ += '\n\t\t';
 for(var j = 0; j < authorities.length; j ++) { 
$_output_ += '\n\t\t';

			var itemUrl = authorities[j];
		
$_output_ += '\n\t\t<li class="item url" data-id="';
$_output_ += itemUrl.id;
$_output_ += '">\n\t\t\t<h4><i class="icon fork"></i>';
$_output_ += itemUrl.url;
$_output_ += '</h4>\n\t\t\t<div class="ui compact labeled icon button btn-url-remove">\n\t\t\t\t<i class="remove icon"></i>\n\t\t\t\t移除\n\t\t\t</div>\n\t\t\t<div>\n\t\t\t\t<div class="name">名称：';
$_output_ += itemUrl.name;
$_output_ += '</div>\n\t\t\t\t<div class="url">描述：';
$_output_ += itemUrl.description;
$_output_ += '</div>\n\t\t\t\t<div class="method">方法：';
$_output_ += itemUrl.method;
$_output_ += '</div>\n\t\t\t</div>\n\t\t</li>\n\t\t';
 } 
$_output_ += '\n\t\t';
 } 
$_output_ += '\n\t</ul>\n</div>';
}
return new String($_output_);

}