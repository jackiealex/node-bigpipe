...
exports.create = function(req, res) {
  ...
  return Q.all([
    ...
    var imageKey, imageObject, imageResult, size, textFields, viewKey, viewObject, viewResult;
    imageResult = results[0];
    viewResult = results[1];
    imageObject = imageResult[0];
    viewObject = viewResult[0];
    if (imageObject['node_code'] !== _CONST_REMOTE_STATUS_CODE_['SUCCESS']) {
      return res.send(imageObject);
    }
    if (imageObject['node_code'] !== _CONST_REMOTE_STATUS_CODE_['SUCCESS']) {
      return res.send(viewObject);
    }
    imageKey = imageObject['data']['resp']['images'][0]['cacheKey'];
    size = imageObject['data']['resp']['images'][0]['size'];
    viewKey = viewObject['data']['resp']['images'][0]['cacheKey'];
    textFields = imageResult[1];
    textFields['cacheKey'] = imageKey;
    textFields['size'] = size;
    textFields['viewKey'] = viewKey;
    return api.request({
      method: 'post',
