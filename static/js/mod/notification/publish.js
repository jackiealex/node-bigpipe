define(['base/draggabilly.pkgd', 'mod/common/widget/suggestion'],function(Draggabilly, Suggestion) {
	...
	function initDatePicker($scope) {
		var date = $scope.find('.datetimepicker');
		date.datetimepicker({
			format: 'Y-m-d H:i',
			lang: 'ch',
			minDate: false,
			...
			onShow: function(ct) {
				...
				this.setOptions({
					minDate: current ? utils.dateFormat(current, '/') : false
				})
			},
			timepicker: true
		});
	...
	function loadDate($scope){
		$scope.on("click", ".clear-timer", function(e){
			$scope.find("input[name=sendTime]").val("");
		});

		$scope.on("click", ".resource-type .activity-detail", function(e){
			$scope.find("input[name=type]").val($(this).data("id"));
			//显示链接填写
			$scope.find(".search-user-tmpl").hide();
			$scope.find(".common-info").show();
		});

		$scope.on("click", ".resource-type .personal", function(e){
			$scope.find("input[name=type]").val($(this).data("id"));
			//显示个人搜索
			$scope.find(".search-user-tmpl").show();
			$scope.find(".common-info").hide();
		});
	}
