define(['base/draggabilly.pkgd', 'mod/common/widget/suggestion', 'mod/common/widget/labelhistory'], function(Draggabilly, Suggestion, LabelHistory) {
    ...
    function onPublish($scope) {
        $scope.on('click', '.btn-submit', function(e) {
            ...
            var $categoryArray = $scope.find('.category-list .category-list-item.selected');

            if($categoryArray.length <=0) {
                return utils.bubble('请选择分类');
            }
            
            var interestcateids = [];
            $categoryArray.map(function(index, item) {
                var id = $(item).data('id');
                interestcateids.push(id);
                return id;
            });
            ...
            var data = {
                ...
                interestIds: interestcateids.join(',')
    ...
    function onTextAreaChange($scope) {
        $scope.on('focus', '.sub-right .textarea-change-event', function(e) {
            var _this = this, textnum, leftnum, evt, code;
            timer = setInterval(function() {
                textnum = $(_this).val().length;
                leftnum = inputLimitCount - textnum;
                if(leftnum < 0) {
                    leftnum = 0;
                    $scope.find('.sub-right .textarea-tips').html("已经超出字数的最大限制!");
                } else {
                    $scope.find('.sub-right .textarea-tips').html('还可输入 <span class="textarea-tips-num" style="font-size: 16px;"> '+leftnum+' </span> 字(共计'+inputLimitCount+'字)');
                }
            } ,400);
        });
    };

    function onLimitInput($scope) {
        $scope.find('.sub-right .textarea-change-event').maxlength({
            'feedback' : ''
        });
    };
